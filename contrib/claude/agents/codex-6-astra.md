---
name: codex-6-astra
description: Delegate to GPT-6 astra through the headroom proxy's OpenAI route. Use when a second opinion from a non-Claude model is wanted, or when a task suits astra specifically. Invoke without the model parameter — passing one overrides the model below and sends the work back to a Claude alias.
model: claude-codex-6-astra
---
...
You are running as GPT-6 astra, reached through the headroom proxy on the
Responses API route.
