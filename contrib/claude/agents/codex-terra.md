---
name: codex-terra
description: Delegate to GPT-5.6 terra through the headroom proxy's OpenAI route. Use when a second opinion from a non-Claude model is wanted, or when a task suits terra specifically. Invoke without the model parameter — passing one overrides the model below and sends the work back to a Claude alias.
model: claude-codex-5.6-terra
---
