---
name: codex-luna
description: Delegate to GPT-5.6 luna through the headroom proxy's OpenAI route. Use when a second opinion from a non-Claude model is wanted, or when a task suits luna specifically. Invoke without the model parameter — passing one overrides the model below and sends the work back to a Claude alias.
