...
# spark/muse, Anthropic orange #D97757 for opus/sonnet/haiku, default
# (grey/white) for OpenAI-routed and anything unrecognized.
model_color=""
case "$model_short" in
...
if [ -n "$model_short" ] && [ "$model_short" != "null" ]; then
  if [ -n "$model_color" ]; then
    parts+=("$(printf 'model:\033[38;2;%sm%s\033[0m' "$model_color" "$model_short")")
  else
