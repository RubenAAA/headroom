...
That's it. Because `CLAUDE_CODE_USE_VERTEX=1` is set, `cclaude --context`
automatically:
...
- **It still goes straight to Google (no savings).** Make sure `CLAUDE_CODE_USE_VERTEX=1`
  is exported *in the same shell* before `cclaude --context`. The launcher only
...
cclaude --context  # with CLAUDE_CODE_USE_VERTEX=1, CLOUD_ML_REGION set
```
