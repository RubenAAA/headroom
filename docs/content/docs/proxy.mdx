...
> Known divergences: the Rust proxy has no `--openai-api-url` (point it at a
> custom OpenAI-compatible upstream with the required `--upstream` base URL,
> per-model `--extra-model-route "name*=URL:openai"`, or the per-request
> `x-headroom-base-url` header) and no `--backend` / `--workers` /
> `--no-optimize` / `--learn`. `--provider-name` exists in both; on the Rust
> proxy the auto-detection reads the configured `--upstream` URL.
...
| `--openai-api-url` | `https://api.openai.com` | Custom OpenAI API URL (**Python proxy only** — the Rust proxy uses `--upstream` / `--extra-model-route` / `x-headroom-base-url`; see note under [CLI options](#cli-options)) |
| `--provider-name` | Detected from the upstream URL | Display name for the OpenAI-compatible upstream on the dashboard (e.g. `OpenRouter`). Well-known hosts (OpenRouter, Groq, Together, Azure OpenAI, …) are detected automatically; this overrides them. Routing and pricing are unaffected. |
...
Claude Code `/compact` continuation summaries are already context, so they never trigger proactive expansion (expanding them would re-add stale session state). Logged as `ccr_expansion_evaluation` with `outcome="compact_summary"`.
...
Deferred turns leave `server_tool_use` / `tool_search_tool_result` blocks in the transcript; side-requests replaying it against a smaller tools array (Stop-hook evaluator, `/compact`) get those blocks repaired automatically so the upstream never 400s.

### Cold-prefix recompaction

When a lane has been idle past the request's real cache TTL, the provider cache is already dead — replaying the prefix byte-identically preserves nothing. With `HEADROOM_COLD_RECOMPACT=1`, the proxy instead skips the splice and, in cache mode, runs lossless whole-prefix folds plus a Spark reasoning-summary strip (unsigned advisory text only; signed thinking and redacted blocks are untouchable). Default off; warm lanes are byte-identical either way.
...
Live session statistics plus durable `persistent_savings` totals. Stored at `~/.headroom/proxy_savings.json` (override with `HEADROOM_SAVINGS_PATH`). The nested `failed_work` aggregate records terminal 5xx requests, attempts and forwarded tokens at risk separately from successful savings; provider usage is included only when the upstream failure reports it.

```bash
curl http://localhost:8787/stats
```

Saved-token figures are headlines: message compression plus tool-schema tokens kept out of context by deferral (`tool_search` block below shows the deferral share). Per-request rows stay message-only by design and carry the additive `tool_schema_saved_tokens` alongside. `by_provider` breaks the recent window down by dashboard display provider (OpenAI-compatible hosts auto-labeled, `--provider-name` overriding).

`recent_requests` (per-request ids, models, errors) is served to loopback callers and explicitly granted dashboard CIDRs only (`HEADROOM_PROXY_TRUSTED_DASHBOARD_CLIENT_CIDRS` plus same-origin provenance); network callers still get every aggregate. `POST /stats/reset` is loopback + same-origin only.
...
Persisted lifetime aggregates. Project names stay loopback-only (or an explicit dashboard-CIDR grant); aggregates are served to everyone.

```bash
curl http://localhost:8787/stats-lifetime
```

### `GET /livez`, `/readyz`, `/health`

Orchestrator probes. `/livez` answers when the process is up; `/readyz` reports readiness with the Kompress soft component listed but excluded from the verdict (a deferred model only degrades plain-text compression to passthrough); `/health` adds the running config (mode, upstream URL).
...
headroom_tool_schema_saved_total 123456
```
