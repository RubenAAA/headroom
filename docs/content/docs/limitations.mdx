...
**Override**: the code compressor (`crates/headroom-core/src/transforms/code_compressor.rs`) is tuned conservatively by default; run `headroom-proxy --help` for the related flags.
