...
> The old Python wrapper (`headroom wrap vscode`) lives in the read-only `upstream-python/` mirror.
...
(The old Python wrapper flag, `headroom wrap vscode --settings-file`, lives in the read-only `upstream-python/` mirror.)
...
future runs need no reconfiguration; while the proxy is
stopped, Copilot requests will fail closed instead of bypassing Headroom.

Remove only Headroom's settings block with:

Remove the marked block Headroom added from VS Code `settings.json`.
...
both `copilot-auth login` and the wrapper setup.

## Verification and troubleshooting

- `headroom copilot-auth status` should report `logged in`.
- `http://127.0.0.1:8787/healthz` should return `{"ok":true}` while the proxy runs.
