...
`cclaude --context`, without changing your Anthropic sign-in or selected model.

<Callout type="info" title="Using Claude Code in a terminal instead?">
Use `cclaude --context`. This page is specifically for Anthropic's official
...
- Headroom installed (`./install.sh`) with the proxy listening on 8787
...
> The old Python wrapper (`headroom wrap vscode-claude`) lives in the read-only `upstream-python/` mirror. `cclaude --context` starts or reuses the Rust proxy and sets `ANTHROPIC_BASE_URL` for you.
...
it with `cclaude --context` before using Claude Code again.

Restore the values that existed before Headroom configured the extension:

Remove the `env` entries the wrapper added from the Claude Code user settings file (`~/.claude/settings.json` on macOS/Linux).
...
(The old Python unwrapper, `headroom unwrap vscode-claude`, lives in the read-only `upstream-python/` mirror.)
