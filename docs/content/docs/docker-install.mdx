...
```bash
curl -fsSL https://raw.githubusercontent.com/RubenAAA/headroom/main/install.sh | bash
```

### macOS (bash 4.3+)

```bash
curl -fsSL https://raw.githubusercontent.com/RubenAAA/headroom/main/install.sh | "$(brew --prefix bash)/bin/bash"
```
...
```powershell
irm https://raw.githubusercontent.com/RubenAAA/headroom/main/install.sh | iex
```
...
Published releases also push versioned GHCR tags such as `ghcr.io/headroomlabs-ai/headroom:0.37.0`, and those images bundle the matching `headroom-proxy` release binary.
...
- `cclaude --context` (Claude Code through the proxy)
- `headroom wrap` flows below live in the read-only `upstream-python/` mirror
...
Use `upstream-python/docker/docker-compose.native.yml` when you want an explicit compose-managed proxy or CLI shell, or when you prefer compose over the native wrapper's `headroom install ...` surface.
...
docker compose -f upstream-python/docker/docker-compose.native.yml up -d proxy
```
...
export HEADROOM_HOST_HOME="$HOME"
export HEADROOM_WORKSPACE="$PWD"
docker compose -f upstream-python/docker/docker-compose.native.yml up proxy
```
...
```bash
docker compose -f upstream-python/docker/docker-compose.native.yml run --rm cli learn
docker compose -f upstream-python/docker/docker-compose.native.yml run --rm cli mcp install
```
