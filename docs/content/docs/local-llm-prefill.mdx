...
> **Which proxy?** The commands below use the Python `headroom proxy` CLI
> (`--openai-api-url`, `--no-optimize`, `--learn`), which is what this
> workflow is validated against. On the Rust proxy (`headroom-proxy`) there is
> no `--openai-api-url` — point it at the local server with the required
> `--upstream <URL>` (it is a pure passthrough unless `--compression` is
> passed, which currently covers Anthropic-shaped traffic only). The optimized
> run below needs the Python proxy until the Rust proxy compresses
