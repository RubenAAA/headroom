...
That's it. Because `CLAUDE_CODE_USE_FOUNDRY=1` is set, `cclaude --context`
automatically:
...
`CLAUDE_CODE_USE_FOUNDRY` is not set in the shell where you ran `cclaude --context`. Make sure to export it before running, or set it in your shell profile.
...
extension may not be installed (`./install.sh` includes it).
