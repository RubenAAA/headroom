...
| `HEADROOM_TOOL_SEARCH_CORE_TOOLS` | Override which tools stay resident under tool-search deferral: unset keeps the built-in core set, empty defers everything non-typed, otherwise a comma-separated list (`toolsearch` always stays resident). Experiment gate for the core-schema deferral trial; read per request, no restart needed | -- |
