...
+---------------------------------------------------------------+
|                           HEADROOM                            |
|    Proxy (headroom-proxy)   ·   Python mirror (upstream-python/) |
|                               |                               |
|                               v                               |
...
Headroom is used through the proxy, which feeds the compression pipeline:
...
| **SDK mode (Python mirror)** | The old `compress()` / `HeadroomClient` path in `upstream-python/` | Minimal — one function call, or a client wrapper |
| **Integrations** | MCP tools at `/mcp/{conversation}`; the old LangChain/Vercel/Agno/Strands/LiteLLM adapters live in `upstream-python/` | Framework-specific setup |

In proxy mode the server is the Rust binary (`crates/headroom-proxy`) with per-provider handlers (Anthropic, OpenAI, Gemini, Bedrock) that each run the same compression pipeline before forwarding.
...
The compressors run natively in the Rust workspace (`crates/headroom-core/src/transforms/`, served by `crates/headroom-proxy`). The ML fallback (**Kompress**, a ModernBERT token compressor) runs through ONNX Runtime, either locally or offloaded to a remote endpoint.
...
See [Cache Optimization](/docs/cache-optimization) for how the two modes interact with the built-in profiles.
