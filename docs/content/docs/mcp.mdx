...
The Rust proxy serves MCP tools at `/mcp/{conversation}` (`headroom_compress`, `headroom_retrieve`, `headroom_stats`). Start the proxy, then point your MCP host at it:

```bash
headroom-proxy --listen 127.0.0.1:8787 --upstream https://api.anthropic.com
```

For automatic compression of **all** traffic, route through the proxy:

```bash
cclaude --context
```

The old Python path (`pip install "headroom-ai[mcp]"`, `headroom mcp install`) lives in the read-only `upstream-python/` mirror.
...
```bash
# (Python mirror only; the Rust proxy serves /mcp/{conversation} directly)
headroom mcp install  # upstream-python/

# Check status (Python mirror)
headroom mcp status  # upstream-python/

# Uninstall
headroom mcp uninstall  # upstream-python/

# Debug mode
headroom mcp serve --debug  # upstream-python/

# Streamable HTTP mode
headroom mcp serve --transport http --host 127.0.0.1 --port 8788 --path /mcp  # upstream-python/
```

## MCP host configuration

For MCP hosts that let you configure a local stdio server, point them at `headroom mcp serve` (Python mirror). If you also run the proxy, pass the proxy URL explicitly so retrieval and stats come from the intended proxy instance.

If you are publishing or consuming Headroom through an MCP registry, use the canonical descriptor at `server.json` in the repo root. It captures the package form for `headroom-ai[mcp]` (Python mirror) and the MCP launch contract in one file.
...
If you need Streamable HTTP instead of stdio, run `headroom mcp serve --transport http` (Python mirror) and point your MCP host at that endpoint — or use the Rust proxy `/mcp/{conversation}` endpoint directly. The default path is `/mcp`, and the default port is `8788`.
...
|------|-------------|-------|
| Claude Code | Native | `/mcp/{conversation}` |
| Cursor | Supported | Add to Cursor MCP settings |
| Codex | If supported | Configure MCP server |
| Any MCP host | Yes | Point to `/mcp/{conversation}` on the proxy |
...
**"MCP SDK not installed"** -- this error comes from the Python mirror (`upstream-python/`); the Rust proxy needs no MCP SDK install.

**"Proxy not running"** -- Start `headroom-proxy` in another terminal (`cclaude --context` does this for you).
...
- Prefer the proxy path for automatic compression of normal Claude Code traffic:
  `headroom-proxy` plus `cclaude --context`.
