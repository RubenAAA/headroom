...
1. **Persistent CCR backend.** `CcrStore` trait gets a `SqliteCcrStore` impl (default) and a `RedisCcrStore` impl (opt-in for multi-worker). The in-memory store stays for tests. rust-dev.md "Multi-worker deployment — CCR fragmentation" section gets updated.
...
- `rust-dev.md` — update "Multi-worker deployment — CCR fragmentation" section: with `SqliteCcrStore` + sticky-session not required; with `RedisCcrStore` no stickiness needed at all.
...
- All new tests pass.
- `rust-dev.md` reflects the new multi-worker story.
