...
`RecacheEvent`:
`at_unix`/`conversation_key`/`session_key_hash`/`drift_dims`/`attribution_reason`/`origin`/`scope`/`replayed_prefix`/`replay_chain_id`/`breakpoints_placed`/`system_markers_dropped`/`previous/forwarded_request_bytes`/`event_kind`/`wasted_tokens`/`cache_creation_input_tokens`/`expected/actual_cache_read`)
...
|---|---|---|---|---|---|
| `inbound_tail_replaced` | `inbound` | `final_message` | `branch` (INFO) | No (shortfall reported as `uncharged_shortfall_tokens`) | Replay declined: only the final inbound message differs — a legitimate branch tail, not a bust. |
...
| `provider_missed_newest_write`, `provider_partial_of_previous_write`, `provider_free_read_not_persisted`, `provider_dropped_older_entry`, `provider_between_entries` | `unknown` | `replayed_prefix` | `unexplained` (WARN) | Yes | Replay applied and the read still came back short: where the read landed against the two previous boundaries (newest write missing, read inside the previous write, free read never persisted, older entry dropped, between entries). Boundary, not a proved provider cause. |
