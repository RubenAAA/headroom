...
async fn cache_hit_rate_emitted_per_session() {
    ...
    // Poll the scrape until the observation lands instead of a fixed
    // settle: the emitter usually follows the drain by ~1ms.
    let scrape = {
        let deadline = tokio::time::Instant::now() + Duration::from_secs(2);
        loop {
            let s = scrape_metrics(&proxy.url()).await;
            if find_value_with_labels(
                &s,
            ...
            assert!(
                ...
                "histogram count never appeared after first session"
            );
