                        ...
                        service_fn(move |_req: Request<hyper::body::Incoming>| async move {
                            ...
                            tokio::spawn(async move {
                                ...
                                for f in frames {
                                    ...
                                    // Yield so the receiver interleaves like a
                                    // live stream, without wall-clock pacing:
                                    // assertions only cover end states.
...
async fn booked_requests(complete: bool, savings_path: std::path::PathBuf) -> i64 {
    ...
    // The truncated turn books nothing, so the snapshot carries no
    // completion signal to poll for — but the task only needs a wake
    // past the drain, which is ~1ms work. 50ms keeps a wide margin.
...
async fn errored_stream_on_5xx_is_booked_failed() {
    ...
    let _ = resp.bytes().await;
    // Poll until the failure is booked instead of a fixed settle: the
    // failed_work counter is the completion signal.
    let snap = {
        let deadline = tokio::time::Instant::now() + Duration::from_secs(2);
        loop {
            let s = probe.snapshot();
            if s["failed_work"]["requests"].as_i64().unwrap_or(-1) == 1 {
                break s;
            }
            assert!(
                tokio::time::Instant::now() < deadline,
                "exhausted 529 never landed in failed work"
            );
