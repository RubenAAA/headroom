...
//!
//! Per `docs/notes/realignment/03-phase-A-lockdown.md`, the `/v1/messages`
...
async fn compression_decision_logged() {
    ...
    // Poll the capture buffer until the decision line lands instead
    // of a fixed settle: the async emitter usually flushes in ~1ms.
    let flushed = common::wait_until(
        std::time::Duration::from_secs(2),
        std::time::Duration::from_millis(5),
        || String::from_utf8_lossy(&buf.lock().unwrap()).contains(r#""decision":"no_change""#),
    )
    .await;
    assert!(flushed, "decision line never flushed");
