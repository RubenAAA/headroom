...
async fn replay_preserves_previous_compressed_provider_prefix() {
    ...
    // Poll fast: the commit usually lands within milliseconds of the
    // drain, so a 50ms pace would always pay 50ms per iteration.
    for _ in 0..100 {
...
async fn a_rebuild_boundary_drops_the_stored_prefix() {
    ...
    // Poll fast (see above): the commit usually lands within milliseconds.
    for _ in 0..100 {
        tokio::time::sleep(Duration::from_millis(10)).await;
        post_turn(&client, &proxy.url(), &turn2).await;
        let got = messages_of(captured.lock().unwrap().last().unwrap());
