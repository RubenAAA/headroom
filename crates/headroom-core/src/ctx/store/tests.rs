...
fn schema_created_and_reopens() {
    ...
    let db = dir.path().join("c.db");
    {
        let store = CtxStore::open(&db).unwrap();
        store
            .index_content("doc", "# Hello\n\nworld body text here", &IndexOpts::default())
            .unwrap();
    }
...
fn index_summary_counts_code_and_prose() {
    ...
    let summary = store.index_content("src", content, &IndexOpts::default()).unwrap();
...
fn reindex_same_label_dedupes() {
    let (_d, store) = open_tmp();
    store.index_content("x", "alpha content here", &IndexOpts::default()).unwrap();
    store.index_content("x", "beta content here", &IndexOpts::default()).unwrap();
    // Old content must be gone.
    let alpha = store
        .search(&["alpha".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
    assert!(alpha.is_empty(), "stale content should be deleted on re-index");
    let beta = store
        .search(&["beta".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
...
fn sanitize_all_stopwords_falls_back() {
    ...
    assert!(!q.contains("\"and\""), "`and` is removed as an FTS operator");
}
...
fn hostile_query_does_not_crash_search() {
    ...
    let hits = store
        .search(
            ...
            &SearchOpts { limit: 5, ..Default::default() },
        )
        .unwrap();
...
fn search_finds_indexed_content() {
    let (_d, store) = open_tmp();
    store
        .index_content("guide", "# Async\n\nspawn_blocking runs on a thread pool", &IndexOpts::default())
        .unwrap();
    let hits = store
        .search(&["spawn_blocking".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
...
fn content_type_filter() {
    ...
    let code = store
        .search(
            &["database".to_string()],
            &SearchOpts { limit: 5, content_type: Some(ContentType::Code), ..Default::default() },
        )
        .unwrap();
    ...
    let prose = store
        .search(
            &["database".to_string()],
            &SearchOpts { limit: 5, content_type: Some(ContentType::Prose), ..Default::default() },
        )
        .unwrap();
...
fn source_filter_literal_substring() {
    let (_d, store) = open_tmp();
    store.index_content("api_docs", "widget endpoint returns json", &IndexOpts::default()).unwrap();
    store.index_content("changelog", "widget bug fixed today", &IndexOpts::default()).unwrap();
    let hits = store
        .search(
            &["widget".to_string()],
            &SearchOpts { limit: 5, source: Some("api".to_string()), ..Default::default() },
        )
        .unwrap();
    assert!(!hits.is_empty());
...
fn timeline_vs_relevance_sort() {
    ...
    store.index_content("early", "recurring keyword alpha", &IndexOpts::default()).unwrap();
    store.index_content("late", "recurring keyword beta", &IndexOpts::default()).unwrap();

    let timeline = store
        .search(
            &["recurring".to_string()],
            &SearchOpts { limit: 10, sort: SortMode::Timeline, ..Default::default() },
        )
        .unwrap();
    ...
    let relevance = store
        .search(
            &["recurring".to_string()],
            &SearchOpts { limit: 10, sort: SortMode::Relevance, ..Default::default() },
        )
        .unwrap();
...
fn fuzzy_correction_recovers_typo() {
    let (_d, store) = open_tmp();
    store
        .index_content("logs", "authentication middleware initialized successfully", &IndexOpts::default())
        .unwrap();
    // "authentcation" (missing 'i') should fuzzy-correct to "authentication".
    let hits = store
        .search(&["authentcation".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
...
fn multi_query_merge_dedupes() {
    let (_d, store) = open_tmp();
    store
        .index_content("doc", "# Cache\n\nprefix cache stability invariants", &IndexOpts::default())
        .unwrap();
    // Two queries both hitting the same chunk — merged, not duplicated.
    let hits = store
        .search(
            &["cache".to_string(), "stability".to_string()],
            &SearchOpts { limit: 10, ..Default::default() },
        )
        .unwrap();
    let mut keys: Vec<String> = hits.iter().map(|h| format!("{}::{}", h.source, h.title)).collect();
    keys.sort();
    let before = keys.len();
    keys.dedup();
...
fn rrf_prefers_document_in_both_layers() {
    ...
    store.index_content("both", "config configuration settings config", &IndexOpts::default()).unwrap();
    store.index_content("onlyone", "settings values here", &IndexOpts::default()).unwrap();
    let hits = store
        .search(&["config".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
    assert_eq!(hits[0].source, "both", "doc matched by both FTS layers ranks first");
}
...
fn opens_ts_created_db() {
    ...
    let store = CtxStore::open(&db).unwrap();
    let hits = store
        .search(&["reciprocal fusion".to_string()], &SearchOpts { limit: 5, ..Default::default() })
        .unwrap();
