...
fn sqlite_ttl_purge() {
    ...
    // Poll until the entry reads back expired instead of a fixed
    // 1100ms: `created_at + ttl_seconds <= now()` flips the moment the
    // unix-second boundary passes, usually well before the full beat.
    let deadline = std::time::Instant::now() + Duration::from_secs(2);
    while store.get(&hash).is_some() {
        assert!(
            std::time::Instant::now() < deadline,
            "expired entry was never purged"
        );
...
fn sqlite_get_refreshes_idle_ttl() {
    ...
    // Go idle past the window. This stay is inherent, not a beat:
    // polling with get() would refresh the idle clock on every hit and
    // keep the entry alive forever, and the unix-second clock needs a
    // full 4s for floor(now) to advance past floor(touch)+3 under `>=`.
